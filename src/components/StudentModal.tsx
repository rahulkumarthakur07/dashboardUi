...
export interface TeacherFormData {
  ...
  classTeacherOf?: string; // optional: store class id if teacher is a class teacher
}
  ...
  const handleAddSubject = () => {
    if (newSubject.classId && newSubject.subjectName) {
      ...
      const exists = teacher.subjects?.some(
        (s) => s.classId === newSubject.classId && s.subjectName === newSubject.subjectName
      );
      if (!exists) {
        setTeacher((prev) => ({
          ...prev,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-6 relative border border-gray-100">
        ...
        <h2 className="text-2xl font-semibold text-gray-800 mb-6">
          {teacher._id ? "Edit Teacher" : "Add Teacher"}
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div className="mt-2">
            <h3 className="font-medium text-gray-700 mb-1">Class Teacher Assignment</h3>
            <select
              value={teacher.classTeacherOf || ""}
              onChange={(e) => handleChange("classTeacherOf", e.target.value)}
              className="w-full border border-gray-200 rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-blue-400 transition"
            >
